  ...
  rateContractorModal.addEventListener("show.bs.modal", function (event) {
    ...
    if (button) {
      ...
      currentContractorName =
        button.getAttribute("data-contractor-name") || "Contractor";
      currentTaskId = button.getAttribute("data-task-id") || "";

      // Set hidden fields
      document.getElementById("ratingContractorId").value = currentContractorId;
      document.getElementById("ratingTaskId").value = currentTaskId;
      document.getElementById("ratingContractorName").value =
        currentContractorName;
...
async function handleRatingSubmission() {
  ...
  if (!contractorId || !taskId) {
    showAlert("Contractor and task information are required.", "danger");
    return;
  }
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        const modal = bootstrap.Modal.getInstance(
          document.getElementById("rateContractorModal")
        );
        if (modal) {
          modal.hide();
        }
      ...
      showAlert(
        result.error || "Failed to submit rating. Please try again.",
        "danger"
      );
...
function openRatingModal(contractorId, contractorName, taskId) {
