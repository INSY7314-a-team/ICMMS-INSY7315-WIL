...
namespace ICCMS_Web.Controllers
{
    ...
    public class AuditLogsController : Controller
    {
        ...
        public async Task<IActionResult> Index(int page = 1, int pageSize = 25, string filterType = "all", string searchTerm = "")
        {
            try
            {
                ...
                var response = await _httpClient.GetAsync($"{_apiBaseUrl}/api/auditlogs?limit=1000");
                
                if (response.IsSuccessStatusCode)
                {
                    var content = await response.Content.ReadAsStringAsync();
                    var apiLogs = JsonSerializer.Deserialize<List<dynamic>>(content, new JsonSerializerOptions
                    {
                    ...
                    if (apiLogs != null)
                    {
                        foreach (var apiLog in apiLogs)
                        {
                            var log = new AuditLogViewModel
                            {
                                ...
                                EntityId = apiLog.GetProperty("entityId").GetString() ?? string.Empty,
                    ...
                    {
                        filteredLogs = filteredLogs.Where(l => l.LogType.Equals(filterType, StringComparison.OrdinalIgnoreCase)).ToList();
                    }
                    ...
                    if (!string.IsNullOrEmpty(searchTerm))
                    {
                        filteredLogs = filteredLogs.Where(l => 
                            ...
                            l.EntityId.Contains(searchTerm, StringComparison.OrdinalIgnoreCase) ||
                            (l.UserFullName != null && l.UserFullName.Contains(searchTerm, StringComparison.OrdinalIgnoreCase))
                        ).ToList();
                    }
                    ...
                    var allLogsResponse = await _httpClient.GetAsync($"{_apiBaseUrl}/api/auditlogs?limit=1000");
                    ...
                    if (allLogsResponse.IsSuccessStatusCode)
                    {
                        ...
                        var allLogsData = JsonSerializer.Deserialize<List<JsonElement>>(allLogsContent, new JsonSerializerOptions
                        {
                            PropertyNameCaseInsensitive = true
                        });
                    ...
                    TempData["Success"] = $"Showing {pagedLogs.Count} of {totalFilteredLogs} audit logs (Page {page} of {totalPages}){filterMessage}{searchMessage}";
